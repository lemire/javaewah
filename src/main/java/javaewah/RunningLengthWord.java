...
public final class RunningLengthWord {
  ...
  /*
   * public void discardFirstWords(long x) { long rl = getRunningLength() ;
   * if(rl >= x) { setRunningLength(rl - x); assert getRunningLength() == rl-x;
   * return; } x -= rl; setRunningLength(0); assert getRunningLength() == 0;
   * final long old = getNumberOfLiteralWords() ; assert old >= x;
   * setNumberOfLiteralWords(old - x); assert old-x ==
   * getNumberOfLiteralWords(); }
   */
