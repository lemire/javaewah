...
public class EWAHCompressedBitmapTest {
	...
	public void SsiYanKaiTest() {
		...
		EWAHCompressedBitmap a = EWAHCompressedBitmap.bitmapOf(39935, 39936,
				...
				39973, 39974, 39975, 39976, 39977, 39978, 39979, 39980, 39981,
				39982, 39983, 39984, 39985, 39986, 39987, 39988, 39989, 39990,
				39991, 39992, 39993, 39994, 39995, 39996, 39997, 39998, 39999,
				...
				40063, 40064, 40065, 40066, 40067, 40068, 40069, 40070, 40071,
				40072, 40073, 40074, 40075, 40076, 40077, 40078, 40079, 40080,
				40081, 40082, 40083, 40084, 40085, 40086, 40087, 40088, 40089,
				40090, 40091, 40092, 40093, 40094, 40095, 40096, 40097, 40098,
				40099, 40100);
		EWAHCompressedBitmap b = EWAHCompressedBitmap.bitmapOf(39935, 39936,
				39937, 39938, 39939, 39940, 39941, 39942, 39943, 39944, 39945,
				39946, 39947, 39948, 39949, 39950, 39951, 39952, 39953, 39954,
				39955, 39956, 39957, 39958, 39959, 39960, 39961, 39962, 39963,
				39964, 39965, 39966, 39967, 39968, 39969, 39970, 39971, 39972,
	...
	public void testMultiAnd() {
		...
		EWAHCompressedBitmap resultBitmap = EWAHCompressedBitmap.and(bitmap1,
				bitmap2, bitmap3);
	...
	public void testHashCode() {
		...
		EWAHCompressedBitmap ewcb = EWAHCompressedBitmap.bitmapOf(50, 70).and(
				EWAHCompressedBitmap.bitmapOf(50, 1000));
		Assert.assertEquals(EWAHCompressedBitmap.bitmapOf(50), ewcb);
		Assert.assertEquals(EWAHCompressedBitmap.bitmapOf(50).hashCode(),
				ewcb.hashCode());
	}
		...
		for (int k = 0; k < 10000; ++k) {
			...
			// AND
			{
				...
				assertEquals(andJdkBitmap,
						EWAHCompressedBitmap.and(ewahBitmap1, ewahBitmap2));
			}
			// MULTI AND
			{
				final BitSet andJdkBitmap = (BitSet) jdkBitmap1.clone();
			...
			// OR
			{
				...
				assertEquals(orJdkBitmap,
						EWAHCompressedBitmap.or(ewahBitmap1, ewahBitmap2));
			...
			// MULTI OR
			{
				...
				assertEquals(orJdkBitmap, EWAHCompressedBitmap.or(ewahBitmap1,
						ewahBitmap2, ewahBitmap3));
				assertEquals(orJdkBitmap, EWAHCompressedBitmap.or(ewahBitmap3,
						ewahBitmap2, ewahBitmap1));
				Assert.assertEquals(orJdkBitmap.cardinality(),
						EWAHCompressedBitmap.orCardinality(ewahBitmap1,
								ewahBitmap2, ewahBitmap3));
			}
		}
	}
