...
public class EWAHCompressedBitmap32Test {
	...
	public void SsiYanKaiTest() {
		...
		EWAHCompressedBitmap32 a = EWAHCompressedBitmap32.bitmapOf(39935,
				39936, 39937, 39938, 39939, 39940, 39941, 39942, 39943, 39944,
				39945, 39946, 39947, 39948, 39949, 39950, 39951, 39952, 39953,
				39954, 39955, 39956, 39957, 39958, 39959, 39960, 39961, 39962,
				39963, 39964, 39965, 39966, 39967, 39968, 39969, 39970, 39971,
				39972, 39973, 39974, 39975, 39976, 39977, 39978, 39979, 39980,
				...
				40017, 40018, 40019, 40020, 40021, 40022, 40023, 40024, 40025,
				40026, 40027, 40028, 40029, 40030, 40031, 40032, 40033, 40034,
				40035, 40036, 40037, 40038, 40039, 40040, 40041, 40042, 40043,
				40044, 40045, 40046, 40047, 40048, 40049, 40050, 40051, 40052,
				40053, 40054, 40055, 40056, 40057, 40058, 40059, 40060, 40061,
				40062, 40063, 40064, 40065, 40066, 40067, 40068, 40069, 40070,
				40071, 40072, 40073, 40074, 40075, 40076, 40077, 40078, 40079,
				40080, 40081, 40082, 40083, 40084, 40085, 40086, 40087, 40088,
				40089, 40090, 40091, 40092, 40093, 40094, 40095, 40096, 40097,
				40098, 40099, 40100);
		EWAHCompressedBitmap32 b = EWAHCompressedBitmap32.bitmapOf(39935,
				39936, 39937, 39938, 39939, 39940, 39941, 39942, 39943, 39944,
				39945, 39946, 39947, 39948, 39949, 39950, 39951, 39952, 39953,
				39954, 39955, 39956, 39957, 39958, 39959, 39960, 39961, 39962,
				39963, 39964, 39965, 39966, 39967, 39968, 39969, 39970, 39971,
	...
	public void testMassiveAnd() {
		...
		Assert.assertTrue(EWAHCompressedBitmap32.and(ewah).getPositions()
				.size() == 0);
	}
	...
	public void testMultiAnd() {
		...
		EWAHCompressedBitmap32 resultBitmap = EWAHCompressedBitmap32.and(
	...
	public void testOrCardinality() {
		...
		for (int N = 0; N < 1024; ++N) {
			...
			if (N + 2 != bitmap.orCardinality(new EWAHCompressedBitmap32())) {
				...
				System.out.println("orCardinality = "
	...
	public void testHashCode() {
		...
		EWAHCompressedBitmap32 ewcb = EWAHCompressedBitmap32.bitmapOf(50, 70)
				.and(EWAHCompressedBitmap32.bitmapOf(50, 1000));
		Assert.assertEquals(EWAHCompressedBitmap32.bitmapOf(50), ewcb);
		Assert.assertEquals(EWAHCompressedBitmap32.bitmapOf(50).hashCode(),
				ewcb.hashCode());
	}
	...
	public static void PolizziTest(int maxlength) {
		...
		for (int k = 0; k < 10000; ++k) {
			...
			for (int pos = 0; pos < len; pos++) { // random *** number of bits
													// set ***
			...
			// MULTI AND
			{
				...
				assertEquals(andJdkBitmap, EWAHCompressedBitmap32.and(
						ewahBitmap1, ewahBitmap2, ewahBitmap3));
				assertEquals(andJdkBitmap, EWAHCompressedBitmap32.and(
						ewahBitmap3, ewahBitmap2, ewahBitmap1));
				Assert.assertEquals(andJdkBitmap.cardinality(),
						EWAHCompressedBitmap32.andCardinality(ewahBitmap1,
			...
			// MULTI OR
			{
				...
				assertEquals(orJdkBitmap, EWAHCompressedBitmap32.or(
						ewahBitmap1, ewahBitmap2, ewahBitmap3));
				assertEquals(orJdkBitmap, EWAHCompressedBitmap32.or(
						ewahBitmap3, ewahBitmap2, ewahBitmap1));
				Assert.assertEquals(orJdkBitmap.cardinality(),
						EWAHCompressedBitmap32.orCardinality(ewahBitmap1,
								ewahBitmap2, ewahBitmap3));
			}
		}
	}
	...
	static void assertEquals(List<Integer> p, int[] v) {
		...
		for (int k = 0; k < v.length; ++k)
			if (v[k] != p.get(k).intValue())
				throw new RuntimeException("expected equal at " + k + " "
						+ v[k] + " " + p.get(k));
	}
	...
	/**
	 * Convenience function to assess equality between a compressed bitset and
	 * an uncompressed bitset
	 * 
	 * @param x
